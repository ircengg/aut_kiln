...
function getAreaRect({ isHorizontal, area, bounds, pitch, rowHeight }) {
  ...
  return {
    x: (area.minTube - 1) * pitch + padding,
    y: bounds.height - area.maxUpper,
...
function PixiCanvas({
  ...
  const rowHeight =
    (isHorizontal ? bounds.width : bounds.height) / Math.max(rowCount, 1);
  const diameter = Math.min(
    wallData.tubeDiameter || pitch * 0.72,
    pitch * 0.92,
  );
  ...
  useEffect(() => {
    ...
    for (let tubeIndex = 1; tubeIndex < tubeCount; tubeIndex += 1) {
      ...
      if (isHorizontal) {
        ...
        graphics.lineTo(position, bounds.height);
      }
    }
    graphics.stroke({
      width: Math.max(1 / zoom, 0.2),
      color: 0x111827,
      alpha: 0.18,
    });
  }, [
  ...
  useEffect(() => {
    ...
    const screenTop = Math.max(0, pan.y);
    ...
    const visibleFirstTube = Math.max(
      0,
      Math.floor((isHorizontal ? -pan.y : -pan.x) / (pitch * zoom)),
    );
    const visibleLastTube = Math.min(
      tubeCount - 1,
      Math.ceil(
        ((isHorizontal ? size.height : size.width) -
          (isHorizontal ? pan.y : pan.x)) /
          (pitch * zoom),
      ),
    );
    const elevationStep = getNiceStep(70 / Math.max(zoom, 0.0001));
    const maxElevation = isHorizontal ? bounds.width : bounds.height;
    ...
    for (
      ...
      if (isHorizontal) {
        ...
        axis.addChild(
          makeAxisLabel(
            String(tubeIndex + 1),
            clamp(screenLeft - 8, 36, size.width - 4),
            y,
            "right",
          ),
        );
        ...
        axis.addChild(
          makeAxisLabel(
            String(tubeIndex + 1),
            x,
            clamp(screenBottom - 12, 12, size.height - 14),
          ),
        );
      }
    }
    ...
    graphics.moveTo(0, screenTop);
    graphics.lineTo(size.width, screenTop);
    ...
    axis.addChild(
      makeAxisLabel(isHorizontal ? "Tube" : "Elevation mm", 42, 14, "left"),
    );
  }, [
    bounds,
    bounds.height,
  ...
  const getCellFromPointer = useCallback(
    (clientX, clientY) => {
      ...
      const elevation = isHorizontal ? worldX : bounds.height - worldY;
      const tubeAxisPosition = isHorizontal ? worldY : worldX;
      ...
      if (tubeOffsetPosition < tubeStart || tubeOffsetPosition > tubeEnd) {
        return null;
      }
